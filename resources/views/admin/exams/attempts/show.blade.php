                ...
                <div class="row g-2 align-items-center">
                    <div class="col">
                        ...
                        <div class="text-secondary">
                            {{ $exam->title }} · Submitted {{ optional($attempt->submitted_at)?->diffForHumans() }}
                        </div>
                    </div>
                    <div class="col-auto ms-auto d-flex gap-2">
                        <a href="{{ route('admin.attempts.show', $exam) }}" class="btn btn-outline">Back to Attempts</a>
                        @php
                            $statusColors = [
                ...
                <div class="card mb-3">
                    <div class="card-body d-flex flex-wrap align-items-center gap-3 justify-content-between">
                        ...
                        <div class="d-flex flex-wrap align-items-center gap-2">
                            <form action="{{ route('admin.attempts.answers.status', [$exam, $attempt]) }}" method="POST"
                                class="d-flex align-items-center gap-2">
                                @csrf
                                @method('PUT')
                                <select name="status" class="form-select form-select-sm">
                                    @foreach ($statusOptions as $statusOption)
                                        <option value="{{ $statusOption }}" @selected($attempt->status === $statusOption)>
                            ...
                            <form action="{{ route('admin.exams.attempts.destroy', [$exam, $attempt]) }}" method="POST"
                                class="d-flex align-items-center gap-2">
                                @csrf
                                @method('DELETE')
                                <input type="text" name="confirm_text" class="form-control form-control-sm"
                                    placeholder="Type Candidate Name" required>
                                <button type="submit" class="btn btn-sm btn-danger">Delete Attempt</button>
                            </form>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-3">
                        <div class="card bg-info text-white">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        <div class="text-uppercase small">Submitted</div>
                                        <div class="fw-semibold">{{ optional($attempt->submitted_at)?->format('M d, h:i a') ?? '—' }}</div>
                                    </div>
